...
def main(logger, args):
    ...
    if args.gpt2 == "llama":
        tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-2-7b-chat-hf", use_auth_token="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")
    else:
        tokenizer = GPT2Tokenizer.from_pretrained(args.gpt2)
    # TODO: Add support for Llama-2
    model = None
...
def run(logger, do_train, do_zeroshot, task, train_task, k, seed,
    ...
    if do_train and (head_tune or not do_check):
        ...
        if not do_check:
            if gpt2=="llama":
                model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-2-7b-chat-hf", use_auth_token="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")
            else:
    ...
    if head_tune:
        print("SPANDAN SHAHRIAR VINAY: WHY ARE WE HERE?\n"*50)
