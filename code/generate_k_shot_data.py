...
def get_label(task, line):
    if task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "STS-B", "WNLI", "CoLA", 'cnli']:
        # GLUE style
...
def split_header(task, lines):
    ...
    if task in ["CoLA"]:
        return [], lines
    elif task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "STS-B", "WNLI", "cnli"]:
...
def main():
    ...
    parser.add_argument("--task", type=str, nargs="+",
        default=['SST-2', 'sst-5', 'mr', 'cr', 'subj', 'trec',
                 'agnews', 'amazon', 'yelp_full', 'dbpedia', 'yahoo', 'cnli'], #, 'mpqa', 'CoLA', 'MRPC', 'QQP', 'STS-B', 'MNLI', 'SNLI', 'QNLI', 'RTE'],
        help="Task names")
    ...
    main_for_gao(args, [task for task in args.task
                        if task in ["SST-2", "sst-5", "mr", "cr", "trec", "subj", 'cnli']])
...
def prepro_for_zhang(dataname, split, seed, args):
    ...
    with open(os.path.join(args.data_dir, "TextClassificationDatasets", DATA_DICT[dataname], "{}.csv".format(split)), "r", encoding='utf-8') as f:
        print(os.path.join(args.data_dir, "TextClassificationDatasets", DATA_DICT[dataname], "{}.csv".format(split)))
